...
fn draw_confirm_back(frame: &mut Frame) {
    ...
    frame.render_widget(
        Block::new()
            .title(Span::styled(" Disconnect ", Style::new().fg(Color::Yellow).add_modifier(Modifier::BOLD)))
            .borders(Borders::ALL)
...
fn draw_statusline(frame: &mut Frame, area: Rect, state: &AppState) {
    ...
    if let Some(ref err) = state.last_error {
        ...
        let msg = if err.len() > max_len { &err[..max_len] } else { err.as_str() };
        let spans = vec![
            Span::styled(" PULSE ", Style::new().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
            Span::styled("  │  ", Style::new().fg(Color::DarkGray)),
            Span::styled(" ✗ ", Style::new().fg(Color::White).bg(Color::Red).add_modifier(Modifier::BOLD)),
    ...
    let mut spans = vec![
        ...
        Span::raw(state.broker.clone()),
        Span::styled("  │  ", Style::new().fg(Color::DarkGray)),
        Span::styled(format!("{} topics", state.topics.len()), Style::new().fg(Color::White)),
        Span::styled("  │  ", Style::new().fg(Color::DarkGray)),
    ...
    if state.paused {
        ...
        spans.push(Span::styled("⏸ PAUSED", Style::new().fg(Color::Yellow).add_modifier(Modifier::BOLD)));
    }

    if !state.search_query.is_empty() {
        spans.push(Span::styled("  │  filter: ", Style::new().fg(Color::DarkGray)));
        spans.push(Span::styled(state.search_query.clone(), Style::new().fg(Color::Cyan)));
    }
...
fn draw_hints(frame: &mut Frame, area: Rect, state: &AppState) {
    let line = if state.yank_mode {
        Line::from(vec![
            Span::styled(" YANK ", Style::new().fg(Color::Black).bg(Color::Yellow).add_modifier(Modifier::BOLD)),
        ...
        Line::from(vec![
            Span::styled(" SUB ", Style::new().fg(Color::Black).bg(Color::Green).add_modifier(Modifier::BOLD)),
        ...
        Line::from(vec![
            Span::styled(" SEARCH ", Style::new().fg(Color::Black).bg(Color::Cyan).add_modifier(Modifier::BOLD)),
            Span::styled("  ", Style::new().fg(Color::DarkGray)),
        ...
        let mut spans = vec![
            ...
            Span::styled(
                ...
                if state.paused { Style::new().fg(Color::Yellow) } else { Style::new().fg(Color::DarkGray) },
            ),
        ];
        if state.paused {
...
fn draw_topics(frame: &mut Frame, area: Rect, state: &AppState, focused: bool) {
    ...
    let items: Vec<ListItem> = state
        ...
        .map(|(i, t)| {
            ...
            ListItem::new(Line::from(vec![
                ...
                Span::styled(
                    if t.tps > 0 { format!("{}t/s", t.tps) } else { String::new() },
    ...
    frame.render_widget(
        List::new(items).block(block.title_bottom(hint)),
        area,
    );
}

fn wrap_line(line: &str, width: usize) -> Vec<String> {
    if width == 0 || line.is_empty() { return vec![line.to_string()]; }
...
fn draw_messages(frame: &mut Frame, area: Rect, state: &AppState, focused: bool) {
    let yank_height: u16 = if state.yank_mode && area.height > 8 { (area.height / 3).max(4) } else { 0 };

    let (list_area, yank_area) = if yank_height > 0 {
        let chunks = Layout::vertical([Constraint::Min(3), Constraint::Length(yank_height)]).split(area);
    ...
    for (idx, msg) in all.iter().enumerate() {
        ...
        let mut meta_spans = vec![
            Span::styled(format!("[{}]", msg.timestamp), Style::new().fg(Color::DarkGray)),
    ...
    let scroll: u16 = if let Some(ci) = state.msg_cursor {
        ...
        if let Some(&sl) = msg_start_line.get(ci) {
            sl.saturating_sub(visible_lines / 3).min(total.saturating_sub(visible_lines)) as u16
        } else { 0 }
    } else {
        total.saturating_sub(visible_lines) as u16
    };

    frame.render_widget(Paragraph::new(all_lines).block(block).scroll((scroll, 0)), list_area);
    ...
    if let (Some(yr), Some(msg)) = (yank_area, state.selected_message()) {
        ...
        let content = if start == end {
            ...
            vec![Line::from(vec![
                Span::raw(msg.payload[..start].to_string()),
                Span::styled(msg.payload[start..end].to_string(), Style::new().bg(Color::Blue).fg(Color::White)),
        ...
        frame.render_widget(Paragraph::new(content).block(yb).wrap(Wrap { trim: false }), yr);
    }
}

fn draw_statusbar(frame: &mut Frame, area: Rect, state: &AppState) {
    let dot_color = if state.connected { Color::Green } else { Color::Red };

    let line = if state.yank_mode {
        Line::from(vec![
            Span::styled(" YANK ", Style::new().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
            Span::styled("  ←→ ", Style::new().fg(Color::Cyan)),
            Span::styled("select   ", Style::new().fg(Color::DarkGray)),
            Span::styled("↑↓ ", Style::new().fg(Color::Cyan)),
            Span::styled("switch msg   ", Style::new().fg(Color::DarkGray)),
